$(function() {
	...
	$('#file-selector').bind('change', function() {
		if(myChart&&myChart.hasOwnProperty('charts')){
			myChart.charts.dispose();
		}
	...
	function createBtnHandler(callback) {
		$.when(fileAnalyze.analyze()).done(function(data) {
			if(data.length > 0 && data[0].hasOwnProperty('省') && data[0].hasOwnProperty('数据')) {
				myChart = new HeatMap('echart', data, $('#chart-title').val());
				$('.zhu-active').hide();
			} else if(data.length > 0 && data[0].hasOwnProperty('省')) {
	...
	$('#re-file').click(function() {
		...
		$('#file-selector').bind('change', function() {
			...
			if(myChart.hasOwnProperty('charts')&&myChart.charts!=null){
				myChart.charts.dispose();
			}
			fileAnalyze = new FileAnalyze(this.files[0]);
			fileUpLoad();
	...
	$('.a-table').click(function() {
		...
		var tableHeight = $('.am-table table').height();
		$('.box-table').width($('.am-table table').width()+58)
					   .height((tableHeight>450?450:tableHeight)+150);
		$('.am-table').height($('.box-table').height()-147);
    })
	// 表格失焦修改图表
